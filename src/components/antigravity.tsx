...
interface AntigravityProps {
  ...
  lerpSpeed?: number;
  color?: string;
  particleColors?: string[]; // New prop for multiple colors
  autoAnimate?: boolean;
...
const AntigravityInner: React.FC<AntigravityProps> = ({
  ...
  const colors = useMemo(() => {
    ...
    for (let i = 0; i < count; i++) {
      ...
      if (particleColors && particleColors.length > 0) {
        c.set(particleColors[Math.floor(Math.random() * particleColors.length)]);
      } else {
        c.set(color);
      }
  ...
  const geometry = useMemo(() => {
    ...
    switch (particleShape) {
      case 'capsule':
        geo = new THREE.CapsuleGeometry(0.1, 0.4, 4, 8);
        break;
      case 'sphere':
